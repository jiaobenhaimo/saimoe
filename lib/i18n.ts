...
const D: Record<string, Entry> = {
  ...
  "rules.subtitle": { zh: "{name}采用如下赛制:提名 → 4 人小组循环赛 → 单败淘汰,选出本届最萌角色。", en: "{name} format: nomination → four-player group round-robin → single-elimination knockout.", ja: "{name}の方式:推薦 → 4人グループ総当たり → 一発勝負トーナメント。" },
  ...
  "rules.s2.p2": { zh: "组内按胜场排名(同分看总得票)。各组前 2 名直接晋级;再按各组「最优的第三名」择优补齐,凑成 16 强或 32 强(视组数而定,晋级数为 2 的幂)。", en: "Within a group, ranking is by wins (ties broken by total votes). The top two of each group advance directly; the best third-placed characters across groups then fill the bracket up to 16 or 32 (a power of two, depending on the number of groups).", ja: "組内は勝ち数(同数は総得票)で順位付け。各組上位2名が直接進出し、さらに各組の最優秀3位でベスト16/32(組数に応じた2のべき乗)まで補います。" },
