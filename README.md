...
- **去重元数据**:每条投票另存一个粗粒度 `device_bucket`(前端 `x-db`,仅用屏幕/时区/核数等跨浏览器稳定的低熵特征,不含 UA/canvas/WebGL)与 `created_at` 时间戳。它**只作元数据供日后运营看板标记**"同设备多浏览器"嫌疑,**绝不用于拦截或去重**(否则会把共用同型号硬件的不同人误并成一个)。
